(() => {
  ...
  function setStatus(text) {
    ...
    const percent = match
      ? Number(match[1])
      : (/LISTO\s*[·-]\s*100%/i.test(text)
          ? 100
          : 0);

    if (exportFill) {
      exportFill.style.width =
        percent + "%";
    }

    if (exportProgress) {
      exportProgress.style.width =
        percent + "%";
    }
  }
  ...
  function fit() {
    ...
    /*
     * IMPORTANTE:
     * canvas.width / height siguen siendo la resolución
     * real de exportación.
     */
    ...
    /*
     * El contenedor del preview adopta exactamente
     * la proporción del formato seleccionado.
     *
     * Esto evita que un canvas vertical quede partido
     * o parezca que hay dos visualizadores.
     */
    ...
    async (event) => {
      ...
      try {
        ...
        setStatus(
          ...
          "Prueba con un MP3 o WAV estándar."
        );
      }
    }
  );
