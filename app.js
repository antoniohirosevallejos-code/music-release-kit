(() => {
  ...
  function planets(energy, t) {
    ...
    // Keep the orbital system visually separated from the title.
    const cy =
      visual.position === "top"
        ? h * .68
        : visual.position === "bottom"
    ...
    // More planets and noticeably faster orbital motion.
    // Each orbit has its own direction, speed and phase so the scene
    // feels alive instead of moving as one rigid system.
    const systems = [
      { rx: .095, ry: .048, speed: .00155, phase: .20, size: .0070, color: 0 },
      { rx: .125, ry: .062, speed: -.00125, phase: 1.40, size: .0090, color: 1 },
      { rx: .155, ry: .076, speed: .00108, phase: 2.20, size: .0065, color: 2 },
      { rx: .185, ry: .091, speed: -.00092, phase: 3.40, size: .0100, color: 0 },
      { rx: .215, ry: .106, speed: .00078, phase: 4.10, size: .0075, color: 1 },
      { rx: .245, ry: .120, speed: -.00068, phase: 5.20, size: .0120, color: 2 },
      { rx: .275, ry: .135, speed: .00058, phase: .90, size: .0080, color: 0 },
      { rx: .305, ry: .150, speed: -.00051, phase: 2.80, size: .0095, color: 1 },
      { rx: .335, ry: .165, speed: .00045, phase: 4.70, size: .0060, color: 2 },
      { rx: .365, ry: .180, speed: -.00040, phase: 1.90, size: .0110, color: 0 },
      { rx: .395, ry: .195, speed: .00035, phase: 3.80, size: .0070, color: 1 },
      { rx: .425, ry: .210, speed: -.00031, phase: 5.60, size: .0085, color: 2 }
    ];

    const energyLevel = Math.max(0, Math.min(1, energy * intensity));
    // Strong enough to "jump" on beats, but capped so planets never fly away.
    const beatPulse = 1 + energyLevel * .075;

    ctx.save();

    // Subtle orbital rings.
    ctx.lineWidth = Math.max(1, base * .0010);
    systems.forEach((s, i) => {
      const rx = base * s.rx;
      const ry = base * s.ry;
      const rot = -.12 + (i % 6) * .045;
    ...
    // A softly pulsing central star gives the system a visual anchor.
    const starR = base * (.014 + energyLevel * .006);
    ...
    star.addColorStop(
      0,
      hexToRgba(p.colors[0], .90 + energyLevel * .08)
    );
    star.addColorStop(
      .28,
      hexToRgba(p.colors[1], .30 + energyLevel * .12)
    );
    ...
    systems.forEach((s, i) => {
      ...
      // Beat expands the orbit slightly — the "jump" requested —
      // while the planet's own radius also pulses with the music.
      const orbitPulse =
        1 + energyLevel * (.035 + (i % 3) * .008);
      ...
      // Small expanding halo on stronger hits.
      if (energyLevel > .18) {
        ctx.strokeStyle = hexToRgba(
          color,
          energyLevel * .20
    ...
    // More background stars, still deterministic and lightweight.
    for (let i = 0; i < 48; i++) {
    ...
    // Occasional deterministic shooting star.
    const cycle = 11;
